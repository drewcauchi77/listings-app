...
<template>
    ...
    <div v-if="$page.props.auth.user.role === 'admin'" class="bg-slate-800 text-white mb-6 p-6 rounded-md font-medium flex items-center justify-between">
        ...
        <button @click="toggleApprove" class="bg-slate-600 px-3 py-1 rounded-md">{{ listing.approved ? 'Disapprove it' : 'Approve it' }}</button>
    </div>

    <Container class="flex gap-4">
        <div class="w-1/4 rounded-md overflow-hidden">
            <img :src="listing.image ? `/storage/${listing.image}` : `/storage/images/listing/default.jpg`" class="w-full h-full object-cover object-center" alt="">
        </div>
            ...
            <div class="mb-6">
                <div class="flex items-end justify-between mb-2">
                    ...
                    <div v-if="canModify" class="pl-4 flex items-center gap-4">
                        ...
                        <button @click="deleteListing" class="bg-red-500 rounded-md text-white px-6 py-2 hover:outline outline-red-500 outline-offset-2" type="button">
                            Delete
                        </button>
                    </div>
                </div>
            ...
            <div v-if="listing.tags" class="mb-6">
                ...
                <div class="flex items-center gap-3 mt-2">
                    <div v-for="tag in listing.tags.split(',')" :key="tag">
                        <Link :href="route('home', { tag })" class="bg-slate-500 text-white px-2 py-px rounded-full hover:bg-slate-700 dark:hover:bg-slate-900">
                            {{ tag }}
                        </Link>
                    </div>
                </div>
